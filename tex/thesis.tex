...
\begin{equation}\label{eq:fhat_univariate}
  ...
  \end{equation}
    %\begin{split}
  %\hat F(z) = \begin{cases}
    %\hat F_{\text{ecdf}}(z) & \text{ if } z \leq v\\
  %\end{cases}
%\end{split}
...
%Now suppose we have some `extreme point' $\vec u$ such that $F(\vec u)$ is close to 1,
%then $-\log F(\vec u) \approx 1 - F(\vec u)$ by Taylor expansion.
%Therefore for $\vec z > 0$

%\[
  %\begin{aligned}
    %\prob(\vec X_i > \vec z + \vec u \big | \vec X_i > \vec u) &= \frac{\prob(\vec X_i > \vec z + \vec u)}{\prob(\vec X_i > \vec u)}\\
     %&= \frac{1 - F(\vec z + \vec u)}{1 - F(\vec u)}\\
     %&\approx \frac{V((\vec z + \vec u - \vec b_n)/\vec a_n)}{V((\vec u - \vec b_n) / \vec a_n)}.\\
  %\end{aligned}
%\]
...
%However~\eqref{eq:basichtapprox} is not quite enough,
%since


%It is necessary to consider $\lim_{z\ra \pm\infty}G(z)$
%in~\eqref{eq:htassump}. Firstly, we must ensure that $\lim_{z\ra +\infty}G(z) =
%1$ (in other words $G$ places no mass at $z=\infty$), since otherwise $a$ and
%$b$ are unidentifiable\cite{keef2013estimation}. Mass at $-\infty$ is
%allowed, but then the approximation~\eqref{eq:basichtapprox} does not make
%sense in practice since $\hat G$ would not be a probability distribution
%function. To make approximation~\eqref{eq:basichtapprox} more sensible, we must
%capture all the `modes' --- intuitively this means that
%there can be several combinations of normalizing functions that satisfy
%~\eqref{eq:htassump}. This has been explored
...
%\subsection{Extremal modes}
%To get an intuition about what these modes look like graphically, see
%Figure~\ref{fig:ar1eg} and Figure~\ref{fig:eth_mode}.
%All data has been converted to Laplace margins, to allow for easy comparison.

%After generating data $\{X_t\}_{t=1}^{5000}$ from an AR(1) process,
%we show the extremal
%dependence between successive values in the series in
%Figure~\ref{fig:ar1eg}. The dependence is positive, and we can see
%only one mode (corresponding to $K=1$ in~\eqref{eq:modalhtapprox}).

%%\footnote{This process is given by $X_{t+1} =  \alpha X_t + \epsilon_t$,
  %%where $\epsilon_t \sim_{\text{iid}} N(0, 1),\,\alpha=0.9$. The expected
  %%(theoretical) RHS of~\eqref{eq:basichtapprox} is therefore $\phi(y-\alpha x)$,
  %%where $\phi$ is the standard Normal pdf. The AR(1) model is special because
  %%approximation~\eqref{eq:basichtapprox} is valid regardless of
  %%the choice of threshold $u$.
%%}.  There
%%is positive dependences between the components of $\vec Y$, and there would
%%be a single mode, i.e. $K=1$ in~\eqref{eq:modalhtapprox}.


%Figure~\ref{fig:eth_mode} shows extremal dependence in Ethereum cryptocurrency
%daily returns. Here the multivariate data is again $\vec Y_t =(X_{t+1},X_{t})$,
%but with $X_t$ now the Ethereum opening price return on day $t$. $K=3$
%suggested modes are shown, but an argument could perhaps be made for $K=2$ or
%$K=3$.

%The modes are determined by the functions $a_k(x)$ and $b_k(x)$ in~
%\eqref{eq:modalhtapprox}, and need not be straight lines as depicted in
%Figure~\ref{fig:eth_mode} (indeed this will only be the case if $a(x) =
%\text{const}$ and $b(x) = \text{const}$).

%\begin{figure}[!htp]
  %\centering
  %\includegraphics[scale=0.75]{../ar/figures/ar1_ht_eg.pdf}
  %\caption{$X_{t+1}$ vs.\ $X_t$ for $X_t > F_{\text{Laplace}}^{-1}(0.9)$, where $X_t$ has been generated by an AR process with parameter $\alpha=0.9$ and standard normal noise, and then transformed to Laplace margins.}\label{fig:ar1eg}
%\end{figure}

%\begin{figure}[!htp]
  %\centering
  %\includegraphics[scale=0.75]{../mode-example/figures/ethereum_modes.pdf}
  %\caption{Consecutive Ethereum cryptocurrency price returns for 2015-08-07 to
  %2022-08-07, with suggested modes. Prices were obtained from
%\href{https://ethereumprice.org}{https://ethereumprice.org}; here $X_t = (o_{t} -o_{t-1})/o_{t-1}$, is the return on the $t$th day, with $o_t$ being the open price on day $t$.}\label{fig:eth_mode}
%\end{figure}
...
\end{enumerate}
%\footnote{As
  %in\cite{tendijck2021modeling}, it may be effective to fit models with a
  %varying number of $K$ modes, and chose the best one via some model selection
%procedure such as DIC or BIC.}
...
%\begin{figure}[htp]
  %\centering
  %\includegraphics[scale=0.75]{../tail-switching/figures/btc_tail_switch.pdf}
  %\caption{BTC price and log returns with tail switches in early 2018. Data collected from Binance.}\label{fig:btcts}
%\end{figure}
...
%\begin{figure}[htp]
  %\centering
  %\includegraphics[scale=0.70]{../asym-log/figures/3dim-basic-realization.pdf}
  %\caption{Realizations of~\eqref{eq:3dimasslogmarkovprocess} on a log scale,
    %conditional on $X_0 > u=F^{-1}_{\text{Fréchet}}(0.995)$ (using the same random seed).}\label{fig:3dimmasslogreal1}
%\end{figure}



%We will generate data $\{X_t\}$ from this model as follows:

%where $F_0$ is the marginal for the first component (i.e. $F_0(x) =
%F(x,\infty,\infty)$), $F_{1|0}(x;x_0) = \p(X_1 \leq x|X_0 =x_0)$, and
%$F_{2|1,0}(x;x_1,x_0) = \p(X_2\leq x|X_1=x_1, X_0 = x_0)$. Derivations for
%these quantities can be found in the \hyperref[appendix:3dimasslog]{appendix},
...
%\subsubsection{Worked example: modelling $\prob(X_{t+1} \leq z | X_{t} = x)$}
...
\begin{table}[htp]
  \centering
  \scalebox{1.0}{
  %\begin{tabular}{p{18mm}|p{15mm}|p{25mm}|p{25mm}|p{25mm}|p{25mm}|p{25mm}}
...
%\begin{table}[htp]
  ...
  %%\begin{tabular}{p{18mm}|p{15mm}|p{25mm}|p{25mm}|p{25mm}|p{25mm}|p{25mm}}
  %\begin{tabular}{p{18mm}|p{15mm}|p{20mm}|p{20mm}|p{20mm}|p{22mm}|p{22mm}}
  %\input{../ht-em/tex/lap_vs_log_e_miae.tex}
%\end{tabular}}\caption{Mean integrated square errors $\pm$ 1 s.d.\ for Laplace and logistic marginals, and for different
%parameter combinations. MISEs are multiplied by 100 for readability.\label{table:lap_vs_log_table1}}
%\end{table}
...
%\FloatBarrier
%\clearpage
%\subsubsection{Monte Carlo Simulation Study}\label{sec:sim_study}


%\begin{enumerate}[(a)]
  %\item place Normal assumptions on
        %$G_k$, i.e. by setting $G_k(z) = \Phi(z;\mu_k,\sigma_k^2)$;
  %\item convert data to Laplace margins (or asymptotically equivalent tail
    %exponential margins) and set $a_i(x)=x^{\beta_i}$, $b(x) = {\alpha_i} x$.
%\end{enumerate}
%(a) was suggested in the original Heffernan-Tawn paper\cite{heffernan2004conditional}, and (b) was suggested in\cite{keef2013estimation}.
%An EM algorithm that solves for $\pi_k$, $\alpha_k$, $\beta_k$, $\mu_k$, and $\sigma_k$,
%$k=1,\ldots,K$ is presented in the \hyperref[appendix:htnormalem]{appendix}.

%For this theoretical model (the asymmetric logistic model), we know
%its conditional distributions {\it exactly}. This will allow us
%to quantify the efficacy of the HT mixture model, by comparison between
%the model and the true distribution.

%TODO simulation study of HT model on $X_{t+1}$ vs. $X_t$, for different thresholds,
%different amounts of data, different values of $v$, $\theta$ etc.
...
%In a later section,~\ref{sec:cevvm_uncertainty}, we discuss uncertainty in
%our CEVMM models.

\subsection{Quantifying error in CEVMM models with bootstrapped confidence
intervals}\label{sec:cevvm_uncertainty}

% theoretical discussion of uncertainties
% discussion of the data used in demo
...
%Uncertainty due to (1) as in
%is taken care of by sampling residuals
%from each of the normal mixtures many times (see Appendix~\ref{appendix:nonparametriccevmm} for the algorithm).
...
%Calculating uncertainty due to (1) is straightforward to implement (see algorithm in appendix TODO),
%but does not capture much of the
%uncertainty (see Figure~\ref{fig:boot_sampling_residuals_only}; the true distribution
%does not reliably lie in the confidence region).


%\begin{figure}[htp]
  %\centering
  %\includegraphics[scale=0.7]{../ht-em/figures/boot-sampling-residuals-only.pdf}
  %\caption{Estimated distribution of $F_{1|0}(\cdot;F_{0}^{-1}(0.99))$ in red.
    %The true distribution is in black. 99\% confidence bounds are in blue,
    %determined only by sampling classifications of the training
  %data.}\label{fig:boot_sampling_residuals_only}
%\end{figure}
...
%We show an example on a realistic dataset generated from the asymmetric
%logistic model. We generate a single time series of length 15000
%using~\eqref{eq:3dimasslogmarkovprocess}, and after moving it to Laplace
%margins, we set $u = F_{\text{Laplace}}^{-1}(0.95)$; see
%Figure~\ref{fig:block_bootstrap_Xs} for a visual representation of the data.
%The other parameters were $\theta = (0.1,0.1,0.1,0.3,0.3,0.3)$, $v = (0.5,0.5,0.5)$.
...
%Normal assumptions on the $G_k$ has been shown to be effective, however they have
%no theoretical motivation. If we could take $G_k$ to be more flexible, then
%it may be possible to apply the HT model more generally.

%One attractive family of distributions, in terms of its flexibility,
%are Bernstein polynomial mixtures. They have been explored extensively (e.g.\ \cite{petrone1999random}),
%but we consider them in following form:

%$$
%\begin{aligned}
  %G(z) &= \sum_{j=1}^{K_B} w_j B(z; j, K_B - j + 1)\\
  %\sum_{j=1}^{K_B} w_j &= 1\\
  %z &\in [0,1]\\
%\end{aligned}
%$$

%where $B(z,a,b)$ are Beta distribution functions and $K_B$ is fixed
%\footnote{Note that $G$ is necessarily a distribution function here
%because $B(\cdot,a,b)$ is a distribution for all $a,b>0$ and
%the weights $w_j$ sum to 1.}. Fitting the mixture weights $w_j$ is seen as an
%effective method for performing nonparametric density
%estimation --- there is an efficient EM
%algorithm for fitting the mixture weights that converges to
%their MLEs\cite{guan2016efficient}. It is even possible to perform
%multivariate density estimation\cite{wang2019bernstein}.

%\subsubsection{Mapping from $\reals$ to $[0,1]$}

%Unfortunately, it is not possible to straightforwardly write
%the $G$'s in~\eqref{eq:modalhtapprox} as Bernstein mixtures, because
%the support of $G$ in general should be $\reals$ but the Beta distribution
%(and hence Bernstein mixtures) are only supported on $[0,1]$.
%One solution is to use $G(z) = \sum_{j=1}^{K_B} w_j B(T(z); j, K_B - j + 1)$
%where $T:\reals \ra [0,1]$ is some suitable monotonically function
%\footnote{
  %Fortunately a large number of such $T$'s are readily available, since any probability
  %distribution function supported on $\real$ is monotonic, and maps to $[0,1]$.
  %Examples include $T_\text{Logistic}(z) = (1+\exp(-z))^{-1}$ and
  %$T_{\text{Normal}}(z) = \Phi(z)$ and $T_{\text{Cauchy}}(z) =
  %\pi^{-1}\arctan(z) + 0.5$.
%}. Under this framework,~\eqref{eq:modalhtapprox} becomes

%\begin{equation}\label{eq:htbernstein}
%\begin{split}
%\prob\left(X_{t+1}\leq y\middle| X_{t} = x\right)
%= & \pi_1 \sum_{j=1}^{K^1_B} w^1_j B\left(T^1\left(\frac{y - b_1(x)}{a_1(x)}\right); j, K^1_B - j + 1\right)\\
%+ &\pi_2 \sum_{j=1}^{K^2_B} w^2_j B\left(T^2\left(\frac{y - b_2(x)}{a_2(x)}\right); j, K_B^2 - j + 1\right)\\
%+ &\dots \\
%+ &\pi_K \sum_{j=1}^{K^K_B} w^K_j B\left(T^K\left(\frac{y - b_K(x)}{a_K(x)}\right); j, K_B^K - j + 1\right)\\
%\text{for large }x,&
%\text{where } \pi,w^1,\ldots,w^K \text{ are probability vectors.}
%\end{split}
%\end{equation}

%The choice of $T$ transformation is critical --- in the ideal case, it spreads
%data evenly over $[0,1]$. If for example $T(z) = (1+\exp(-z))^{-1}$ is chosen
%regardless of the data, Bernstein mixtures can act as very poor density
%estimators. See Figure~\ref{fig:bernstein_normal_demo}; $\mu=0$ performs well
%because the sigmoid function $(1+\exp(-z))^{-1}$ spreads out standard normal
%data nicely over $[0,1]$, but $\mu=5$ performs badly because $N(5,1)$ data is
%`squashed' to the upper end of $[0,1]$.


%\begin{figure}[htp]
  %\centering
  %\includegraphics[scale=0.75]{../ht-bernstein/figures/bernstein_normal_T_graph.pdf}
  %\caption{Bernstein mixtures fit to 3 ($\mu=0,2.5,5$) datasets,
  %$\{X_{i}\}_{i=1}^{500}\sim_{\text{i.i.d.}}N(\mu,1)$. The histograms of the datasets are shown in orange, with the fitted Bernstein mixture density estimators (after transforming back  from $[0,1]$ to $\reals$, for easier comparison) in blue.
%The number of Bernstein mixtures used for each fitting was 20 (i.e.\ $K_B=20$).}\label{fig:bernstein_normal_demo}
%\end{figure}

%In~\eqref{eq:htbernstein}, the functions $a_i$ and $b_i$ are unknown
%and therefore it is very difficult to choose $T^k$ that perform well, because
%the range of the data is not even known.


%\section{Fitting Heffernan-Tawn 6-mixture models to asymmetric logistic data}

%So far we tried to model the tails of process~\eqref{eq:3dimasslogmarkovprocess},
%but only looking at the 1st order dependence. We can additionally use the HT
%framework to model the distribution of $X_{t+2} | X_{t+1},X_{t}$, where either
%one of $X_{t+1}$ or $X_{t}$ are extreme. By `$X$ extreme', as before, it is meant
%that $X > u$ for some large threshold $u$. This leads to the following $2^3$ cases

%\begin{enumerate}[(1)]
  %\item $X_{t+2},X_{t+1},X_{t}$ are all extreme,
  %\item $X_{t+2},X_{t+1}$ are extreme, but $X_{t}$ is not extreme,
  %\item $X_{t+2},X_{t}$ are extreme, but $X_{t+1}$ is not extreme,
  %\item $X_{t+1},X_{t}$ are extreme, but $X_{t+2}$ is not extreme,
  %\item $X_{t+1}$ is extreme, but $X_{t+2},X_{t}$ are not extreme,
  %\item $X_{t}$ is extreme, but $X_{t+2},X_{t+1}$ are not extreme,
  %\item $X_{t+2}$ is extreme, and $X_{t+1}$ and $X_{t}$ are not extreme,
  %\item None of $X_{t+2},X_{t+1},X_{t}$ are extreme.
%\end{enumerate}

%Cases (7) and (8) are ignored since they contain no extremes and therefore
%it does not make sense to analyse them using extreme value methods. We are left
%with 6 cases.
%This partitioning of the random variables into extreme and non-extreme regions
%suggests the following Heffernan-Tawn inspired mixture model:

%\begin{equation}\label{eq:6assloghtmodel}
  %\begin{split}
    %&\prob\left(X_{t+2}\leq x\middle| X_{t+1}=x_{t+1},X_{t} = x_t, \max(X_{t+1},X_{t}) > u\right)=\\
     %& \pi_1 G_1\left((x - b_1(x_{t+1},x_{t}))/a_1(x_{t+1},x_t)\right)\\
    %+& \pi_2 G_2\left((x - b_2(x_{t+1}))/a_2(x_{t+1})\right)\\
    %+& \pi_3 G_3\left((x - b_3(x_{t}))/a_3(x_t)\right)\\
    %+& \pi_4 G_4(x)\\
    %+& \pi_5 G_5(x)\\
    %+& \pi_6 G_6(x),\\
  %\end{split}
...
%where the mixture components $G_k$ for $k=1,\ldots,6$ correspond to cases above.
%The intuition for this model is that $G_1,\ldots,G_3$ capture possibilities
%where the process remains in the tails, and $G_4,\ldots,G_6$ capture the possibilities
%where we revert back to the `main body' of the process.


%If similar assumptions are to be made as before (i.e. Normal assumptions on
%$G_k$; $a_k(x) = x^{\beta_k}$ and $b(x) = {\alpha_k} x$), then it is easy to
%fit components $k=2,\ldots,6$ using a version of Algorithm~\ref{alg:normalht}
%(with slight modifications to force $\alpha_4,\alpha_5,\alpha_6=0$ and
%$\beta_4,\beta_5,\beta_6=1$).

%Unfortunately, the 1st mixture component, $G_1$, provides a complication.
%While the functions $a_1:\mathbb{R}^2\rightarrow \mathbb{R}$ and $b_1:\mathbb{R}^2\rightarrow \mathbb{R}$
%can be found analytically for the asymmetric logistic process, a general
%multivariate 3-dimensional probability distribution has no nice form
%(TODO what are the specifics here? + provide citation to Ioannis' paper).
%It is considered later how to deal with this, but one solution is to
%fit separately a model

%\begin{equation}
  %\begin{split}
    %\prob\left(X_{t+2}\leq x\middle| X_{t+1}=x_{t+1},X_{t} = x_t, \min(X_{t+1},X_{t}) > u\right) \approx  H(x;x_{t+1},x_{t})
  %\end{split}
%\end{equation}

%on a reduced set of data ($\mathcal{D} = \{(X_{t+2},X_{t+1},X_{t}) | X_{t+2}>u,X_{t+1}>u,X_{t}>u\}$),
%treat\\
%$G_1\left((x - b_1(x_{t+1},x_{t}))/a_1(x_{t+1},x_t)\right) = H(x;x_{t+1},x_t)$ as fixed,
%and then fit the remaining parameters in~\eqref{eq:6assloghtmodel}.

%TODO fit~\eqref{eq:6assloghtmodel}, both with the analytic form for $a_1$, $b_1$,
%and additionally with the non-parametric
...
%We also check the autocorrelation
%an ACF plot: see Figure~\ref{fig:acf_residuals}. While the autocorrelations
%are statistically significant, they are at least small.

%\begin{figure}[htp]
  %\centering
  %\includegraphics[scale=0.75]{../elexon/figures/acf_residuals.pdf}
  %\caption{ACF plot of $y :=\code{log_price} - \widehat{\code{log_price}}$.}\label{fig:acf_residuals}
%\end{figure}
...
\end{figure}
%As in~\ref{sec:cevvm_uncertainty}, we select our threshold for exceedances
%to be $u=F_{\text{Laplace}}^{-1}(0.95)$.
...
\begin{table}[htp]\centering
  \input{../elexon/tex/seed_18.tex} \caption{Best CEVMM fit of 30 random initial values, with no regularization.\label{table:seed_18} }
\end{table}
...
%We are now in the position to fit a CEVMM. We begin with $K=2$ and initially
%with no regularization. The lack of regularization leads to overfitting in the
%upper mode -- see Figure~\ref{fig:seed_18}


%Figure~\ref{fig:imb_contour_with_reg05} shows a more promising fit, though
%there are some problems. Firstly, there is a data point classified
%seemingly incorrectly (see the green data point in the top left of the Figure).
%This is because the data point, $(y_t,y_{t+1})=(2.44, 6.77)$, is far away from
%either
%of the two Gaussian modes, meaning it has a very small probability of belonging
%to either; but the mode with the highest variance ($\hat\sigma_2$ in
%Table~\ref{table:imb_reg05}) `wins'; in other words, it is an outlier.
%Secondly, the green cluster is smaller than we might expect --- it appears
%to contain fewer points than the red. This can
%be explain by the fitted parameter values in Table~\ref{table:imb_reg05}, and noting
%that the proportion of data points assigned to the positive dependence cluster (in red) is 0.52. The discrepancy in $\hat\pi = 0.43$ and 0.52 is due to the way
%$\hat\pi$ is updated in~\ref{alg:normalht} --- it takes
%into account the likelihoods of each data point and doesn't simply look at the
...
%Figure~\ref{fig:imb_first_order_k_eq_3}, we fit Heffernan-Tawn models for $K=2$
%and $K=3$, and show contours for the models\footnote{
  %These contours should be taken liberally, as it is usual practice to
  %discard the contours once the model has been fit.
%}
...
%by performing one-step-ahead residual prediction with January 2022 imbalance price residuals.


%\begin{enumerate}[1]
  %%\item Sample $
  %%\item Sample sets of residuals $M_1$ times to form $M_1$
    %%CEVMM models.
  %%\item For each of the $M_1$ models, account for classification uncertainty by sampling $M_2$ sets
    %%of classifications.
  %\item Draw a sample of $y_{t+1}$ from a random selection of one of the $M_1$
    %models, conditional on $y_t = y$. Repeat $M_2$ times to get $M_2$
    %samples.
  %\item Use all $M_2$ samples to form an estimated distribution for the next price's residual $y_{t+1}$.
  %%\item Finally, use model~\ref{fig:stepwise_output_lm} to convert the residual $y_{\text{next}}$ into a price estimate.
%\end{enumerate}


%This
%is a good test case because the (1) the data is out of sample,
%and (2) the true residuals are known, so it is possible to assess the
%performance of our predictions.
...
%\section{3-dimensional asymmetric logistic model}
%\label{appendix:3dimasslog}

%Here we present derivations of the conditional distributions $F_{1|0}$,
%$F_{2|1,0}$, where $F=\exp(-V)$ is as in~\eqref{eq:3dimasslog} (with the
%assumptions of $\theta_{01}=\theta_{12}$ and $v_{01}=v_{12}$ for stationarity
%of~\eqref{eq:3dimasslogmarkovprocess}).

%Some preliminary results are

%\begin{equation}
  %\begin{split}
    %-x_0^{2}\frac{\partial V}{\partial x_0} &= \theta_0\\
  %&+ \theta_{01}x_0^{1-1/v_{01}}\left(x_0^{-1/v_{01}} + x_1^{-1/v_{01}}\right)^{v_{01}-1}\\
  %&+ \theta_{02}x_0^{1-1/v_{02}}\left(x_0^{-1/v_{02}} + x_2^{-1/v_{02}}\right)^{v_{02}-1}\\
  %&+ \theta_{012}x_0^{1-1/v_{012}}\left(x_0^{-1/v_{012}}+x_1^{-1/v_{012}} + x_2^{-1/v_{012}}\right)^{v_{012}-1}\\
  %\end{split}
%\end{equation}

%\begin{equation}
  %\begin{split}
    %-x_1^{2}\frac{\partial V}{\partial x_1} &= \theta_1\\
    %&+ \theta_{01}x_1^{1-1/v_{01}}\left(x_0^{-1/v_{01}} + x_1^{-1/v_{01}}\right)^{v_{01}-1}\\
  %&+ \theta_{01}x_1^{1-1/v_{02}}\left(x_1^{-1/v_{01}} + x_2^{-1/v_{01}}\right)^{v_{01}-1}\\
  %&+ \theta_{012}x_1^{1-1/v_{012}}\left(x_0^{-1/v_{012}}+x_1^{-1/v_{012}} + x_2^{-1/v_{012}}\right)^{v_{012}-1}\\
  %\end{split}
%\end{equation}

%\begin{equation}
  %\begin{split}
    %\frac{\partial^2 V}{\partial x_0 \partial x_1} &=
    %\theta_{01}v_{01}^{-1}x_0^{-1-1/v_{01}}x_1^{-1-1/v_{01}}\left(x_0^{-1/v_{01}} + x_1^{-1/v_{01}}\right)^{v_{01}-2}\\
     %&+ \theta_{012}v_{012}^{-1} x_0^{-1-1/v_{012}} x_1^{-1-1/v_{012}}\left(x_0^{-1/v_{012}}+x_1^{-1/v_{012}} + x_2^{-1/v_{012}}\right)^{v_{012}-2}\\
  %\end{split}
%\end{equation}
...
%\[
  %\begin{aligned}
    %F_{1|0}(x;x_0) &=& \prob(X_1 \leq x|X_0 = x_0) =
    %\frac{(\partial_{x_0} F)|_{x_0=x_0,x_1=x,x_2=\infty}}
    %{(\partial_{x_0} F)|_{x_0=x_0,x_1=\infty,x_2=\infty}}\\
                   %&=& -x_0^2 (F\partial_{x_0} V)|_{x_0=x_0,x_1=x,x_2=\infty}\\
                   %&=& \exp(1/x_0 - V(x_0, x_1, \infty))\{\theta_0 + \theta_{02} + \theta_{01}x_{0}^{1-1/v_{01}}(x_0^{-1/v_{01}} + x_1^{-1/v_{01}})^{v_{01}-1}\\
                   %&&+\theta_{012}x_{0}^{1-1/v_{012}}(x_0^{-1/v_{012}} + x_1^{-1/v_{012}})^{v_{012}-1}\}
  %\end{aligned}
%\]
...
%\[
  %\begin{aligned}
    %F_{2|1,0}(x;x_1,x_0) &=& \prob(X_2 \leq x|X_1=x_1,X_0 = x_0) =
    %\frac{(\partial^2_{x_0,x_1} F)|_{x_0=x_0,x_1=x_1,x_2=x}}
    %{(\partial^2_{x_0,x_1} F)|_{x_0=x_0,x_1=x_1,x_2=\infty}}\\
   %&=&\frac{\left[(\partial_{x_0} V) (\partial_{x_1} V)-\partial^2_{x_0,x_1}V\right] F|_{x_0=x_0,x_1=x_1,x_2=x}}
    %{\left[(\partial_{x_0} V) (\partial_{x_1} V)-\partial^2_{x_0,x_1}V\right] F|_{x_0=x_0,x_1=x_1,x_2=\infty}}\\
  %\end{aligned}
%\]

%TODO $F_{2|1,0}(x;x_1,x_0)$


%\clearpage
%\section{Appendix: EM algorithm for fitting a simple 2-mixture model}
%\label{appendix:simpleem}

%Assume the following model, where all of $G

%\[
    %\p\left(Y\leq y | X =x,right) =
    %\pi G_1\left(\frac{y - b_1(x)}{a_1(x)}\right)
  %+ (1-\pi)G_2\left(\frac{y - b_2(x)}{a_2(x)}\right).
%\]

%Since everything but $\pi$ is known, we can equivalently work with
%the following (making the known quantities implicit):
...
%where $H_i$ are known probability distribution functions in $y$.
%Write $h_i(\cdot,x)$ for the associated PDFs, then the likelihood
%of the data $\D=\{(x_i, y_i)\}_{i=1}^n$ (where $x_i > u$, for some
%ideally large threshold $u$) is

%\[
%L(\pi) = \prod_{i=1}^n \left(\pi h_1(y_i, x_i) + (1-\pi) h_2(y_i,x_i)\right).
%\]

%Additionally, let $Z\in\{0,1\}^n$, be a vector of indicator variables, with
%$Z_i=1$ if the $i$th observation is drawn from the first mixture component.
%The corresponding conditional likelihood and log-likelihoods are:

%\[
  %\begin{aligned}
    %L(\pi|Z) &= \prod_{i=1}^n \left[\pi h_1(y_i, x_i)\right]^{Z_i}
  %\left[(1-\pi)h_2(y_i,x_i)\right]^{1-Z_i}\\
    %l(\pi|Z)=\log L(\pi|Z) &= \sum_{i=1}^n \left\{Z_i\log\pi + (1-Z_i)\log(1-\pi) + Z_i\log h_1(y_i,x_i) + (1-Z_i)\log h_2(y_i,x_i)\right\}
%\end{aligned}
%\]

%An efficient EM
%algorithm for fitting $\pi$ is as follows.

%\begin{algorithm}
  %\caption{EM algorithm for fitting a simple 2-mixture model\label{alg:simpalg}}
%\begin{algorithmic}
%\State Choose some initial value for $\pi$, $\pi^{(0)}$ (e.g. 0.5).
%\State $t \gets 0$
%\While{$\pi^{(t)}$ not converged}
  %\For{$i \in 1,\ldots, n$}
  %\State $\tilde\pi_i  \gets \frac{\pi^{(t)} h_1(y_i, x_i)}{\pi^{(t)} h_1(y_i, x_i) + (1-\pi^{(t)})h_2(y_i, x_i)}$
  %\EndFor
  %\State $\pi^{(t+1)} \gets \frac{1}{n}\sum_{i=1}^n \tilde\pi_i$
  %\State $t \gets t + 1$
%\EndWhile
%\State \Return $\hat\pi = \pi^{(t)}$
%\end{algorithmic}
%\end{algorithm}
